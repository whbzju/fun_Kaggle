...
df_train_pc = pd.read_csv('train_app_clean.csv')

clf = trainModel(df_train_pc, 'app')

from sklearn.externals import joblib
joblib.dump(clf, 'model/gbdt100App.pkl') 

df_test_pc = pd.read_csv('test_app_clean.csv')
df_test_pc_id = pd.read_csv('test_app_id.csv')

y = predict(clf, df_test_pc)
save('gbdt_test_app_prediction', df_test_pc_id, y)
